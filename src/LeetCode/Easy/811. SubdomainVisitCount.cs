...
public class SubdomainVisitCount
{
    public IList<string> SubdomainVisits(string[] cpdomains)
    {
        ...
        {
            string[] input = cpdomain.Split(' ');
            int value = Convert.ToInt32(input[0]);
            string results = "";
            string[] strs = input[1].Split('.');
            for (int i = strs.Length - 1; i >= 0; i--)
            {
                if (results == "")
                    results = strs[i] + results;
                else
                    results = strs[i] + "." + results;


                if (domains.ContainsKey(results))
                {
                    domains[results] += value;
                }
                else
                {
                    domains.Add(results, value);
                }
            }
        }
